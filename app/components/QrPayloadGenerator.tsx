...
export default function QrPayloadGenerator({onChange,details,setData}: QrPayloadGeneratorProps) {
  const debounce = useRef<Nullable<NodeJS.Timeout>>(null);
  const [qrPayloadDetails, setQRPayloadDetails] = useState<QrPayloadData>(details);
  ...
  return <>
    ...
    {qrPayloadDetails.type === 'text' ? <>
      ...
      <Input id="text" type="text" placeholder="Enter text" value={qrPayloadDetails.text} onChange={(e) => handleQRDetailChange('text', e.target.value)} />
    </>:null}

    {qrPayloadDetails.type === 'url' ? <>
      <Label htmlFor="url">Url</Label>
      <Input id="url" type="text" placeholder="Enter URL" value={qrPayloadDetails.url} onChange={(e) => handleQRDetailChange('url', e.target.value)} />
    </>:null}

    {qrPayloadDetails.type === 'wifi' ? <>
      <Label htmlFor="ssid">Ssid</Label>
      <Input id="ssid" type="text" placeholder="Enter SSID" className="mb-2" value={qrPayloadDetails.ssid} onChange={(e) => handleQRDetailChange('ssid', e.target.value)} />

      <Label htmlFor="password">Password</Label>
      <Input id="password" type="password" placeholder="Enter Password" className="mb-2" value={qrPayloadDetails.password} onChange={(e) => handleQRDetailChange('password', e.target.value)} />

      <Label htmlFor="encryption">Encryption</Label>
      <Select key="encryption" onValueChange={(e) => handleQRDetailChange('encryption', e)} >
        <SelectTrigger id="encryption" className="mb-2">
          <SelectValue placeholder={qrPayloadDetails.encryption} />
        </SelectTrigger>
        <SelectContent>
          <SelectItem key="WPA2" value="WPA2">WPA2</SelectItem>
      ...
      <div className="flex items-center">
        ...
        <Checkbox id="hidden" className="ml-2" checked={qrPayloadDetails.hidden} onCheckedChange={(e) => handleQRDetailChange('hidden', e)} />
      </div>
    </>:null}
  </>
};